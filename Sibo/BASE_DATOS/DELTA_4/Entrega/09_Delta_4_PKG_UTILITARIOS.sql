  ...
  ******************************************************************************/
  ...
  AS
    ...
    BEGIN
      ...
      PAGO_DAVIPLATA CONSTANT INTEGER := 1;
      PAGO_GIRO CONSTANT INTEGER := 2;
      ...
      BEGIN
        ...
        SELECT IDENTIFICADOR INTO  C_OTROS FROM TIPO_ARCHIVO WHERE TABLAINSERCION = 'OTROS';
        SELECT IDENTIFICADOR INTO  C_MFEA FROM TIPO_ARCHIVO WHERE TABLAINSERCION = 'MFEA';
        SELECT IDENTIFICADOR INTO  C_UARIV FROM TIPO_ARCHIVO WHERE TABLAINSERCION = 'UARIV';
        ...
        BEGIN
          ...
          IF P_TIPARCHIVO  = C_OTROS THEN
            FOR OT IN CUR_REG_OTROS
            LOOP
              IF OT.TIPO_PAGO      = PAGO_DAVIPLATA THEN
                TOTAL_DAVIPLATA   := OT.TOTAL_REGISTROS;
                P_TOTAL_DAVIPLATA := OT.TOTAL_PAGO;
              ELSIF OT.TIPO_PAGO   = PAGO_GIRO THEN
              ...
              END IF;
            END LOOP;
          ELSIF P_TIPARCHIVO = C_UARIV THEN
            FOR AU IN CUR_REG_UARIV
            LOOP
              IF AU.TIPO_PAGO      = PAGO_DAVIPLATA THEN
                TOTAL_DAVIPLATA   := AU.TOTAL_REGISTROS;
                P_TOTAL_DAVIPLATA := AU.TOTAL_PAGO;
              ELSIF AU.TIPO_PAGO   = PAGO_GIRO THEN
              ...
              END IF;
            END LOOP;
          ELSIF P_TIPARCHIVO = C_MFEA THEN
            FOR MF IN CUR_REG_MFEA
            LOOP
              IF MF.TIPO_PAGO      = PAGO_DAVIPLATA THEN
                TOTAL_DAVIPLATA   := MF.TOTAL_REGISTROS;
                P_TOTAL_DAVIPLATA := MF.TOTAL_PAGO;
              ELSIF MF.TIPO_PAGO   = PAGO_GIRO THEN
