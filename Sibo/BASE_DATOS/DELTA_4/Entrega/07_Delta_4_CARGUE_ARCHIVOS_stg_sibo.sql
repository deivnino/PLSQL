...
create or replace PACKAGE BODY PKG_CARGUE_ARCHIVOS
  ...
  AS
    ...
    CAMPO_CONCATENA_CADENA_ERROR            := P_CAMPO_CONCATENA_CADENA_ERROR;
      ...
      CLOB_PRC_DINAMICO := CLOB_PRC_DINAMICO || '      SQL_DINAMICO_INSERT               := SQL_DINAMICO_INSERT || ''('' || SEC_RESUMEN_CARGUE|| '','' || TIPO_ARCHIVO ||'',SYSDATE, TO_CHAR(SYSDATE, ''''hh:mi:ss''''),'' || CHR(10);' ||CHR(10);
...
CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || '                             P_FECHA_PROVISTA                IN VARCHAR,--									' ||CHR(10);
...
CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || ' P_NOMBRE_PARAMETRO_ERROR                VARCHAR2(100);                                                        ' ||CHR(10);
CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || ' P_NOMBRE_PARAMETRO_BAD                  VARCHAR2(100);                                                        ' ||CHR(10);
CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || ' PREFIJO_ERROR                           VARCHAR2(100);                                                       ' ||CHR(10);
CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || ' NOMBRE_BAD                              VARCHAR2(100);                                                       ' ||CHR(10);
CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || ' FECHA_PROVISTA                          VARCHAR2(20); --														' ||CHR(10);
CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || ' 																												' ||CHR(10);
...
CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || ' NOMBRETABLA_ERR                         VARCHAR2(100);                                                       ' ||CHR(10);
CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || ' NOMBRETABLA_BAD                         VARCHAR2(100);                                                       ' ||CHR(10);
CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || ' SEC_ERROR_CARGUE                        VARCHAR2(200);                                                       ' ||CHR(10);

CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || ' DATOS_BAD                               INTEGER;                                                             ' ||CHR(10);
CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || ' DATOS_ERROR                             INTEGER;                                                             ' ||CHR(10);

CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || '   deadlock_detected EXCEPTION;                                                                               ' ||CHR(10);
CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || 'PRAGMA EXCEPTION_INIT(deadlock_detected, -942);                                                               ' ||CHR(10);
...
CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || '    NOMBRE_BAD                     := P_TABLA_BAD;  --                                                        ' ||CHR(10);
CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || '    FECHA_PROVISTA                 := P_FECHA_PROVISTA;--                                                     ' ||CHR(10);
CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || '																																	' ||CHR(10);
CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || '       P_NOMBRE_PARAMETRO_ERROR   := PREFIJO_ERROR;                                                                               ' ||CHR(10);
CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || '       P_NOMBRE_PARAMETRO_BAD    := NOMBRE_BAD;--                                                                                 ' ||CHR(10);
CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || '                                                                                                                                  ' ||CHR(10);
CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || '   EXCEPTION                                                                                                                      ' ||CHR(10);
CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || '   WHEN OTHERS THEN                                                                                                               ' ||CHR(10);
CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || '   PKG_UTILITARIOS.REPORTA_ERROR(''ERROR EN SELECT A PARAMAETROS: '', SQLERRM);                                                     ' ||CHR(10);
CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || '   END;                                                                                                                           ' ||CHR(10);
CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || '                                                                                                                                  ' ||CHR(10);
CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || '   NOMBRETABLA_ERR                       :=  P_NOMBRE_PARAMETRO_ERROR || TRIM(FECHA_PROVISTA);                                    ' ||CHR(10);
...
------------------------
      ...
      CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || '   PKG_UTILITARIOS.REPORTA_ERROR(''SELECT: '', QUERY_ERROR);' ||CHR(10);
      CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO ||   CHR(10);
      CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || '   QUERY_BAD                             :=  ''SELECT 1 AS IDCARGUE, 1 AS IDERRORLINEACARGUE, REGISTRO, ''''No se reporta error'''' AS DESCRIPCIONERROR FROM '' || NOMBRETABLA_BAD;' ||CHR(10);
      CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || '   --DBMS_OUTPUT.PUT_LINE(QUERY_BAD);' ||CHR(10);
      
      CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || 'BEGIN' ||CHR(10);
      CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || '   EXECUTE IMMEDIATE ''SELECT COUNT(*) FROM '' || NOMBRETABLA_ERR INTO DATOS_ERROR;' ||CHR(10);
      CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || 'EXCEPTION WHEN deadlock_detected THEN' ||CHR(10);
      CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || 'DATOS_ERROR := 0;' ||CHR(10);
      CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || 'END;' ||CHR(10);
    
      CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || '   IF DATOS_ERROR IS NOT NULL AND DATOS_ERROR <> 0  THEN' ||CHR(10);
      ...
      CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || '                 NULL);' ||CHR(10);
      CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || '           EXCEPTION WHEN OTHERS THEN' ||CHR(10);
      CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || '         PKG_UTILITARIOS.REPORTA_ERROR(''ERROR EN ERROR:  '', '' DESCRIPCION: '' || SQLERRM);' ||CHR(10);
      CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || '       END;' ||CHR(10);
      CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || '   END LOOP;' ||CHR(10);
      ...
      CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || 'BEGIN' ||CHR(10);
      CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || ' EXECUTE IMMEDIATE ''SELECT COUNT(*) FROM '' || NOMBRETABLA_BAD INTO DATOS_BAD;' ||CHR(10);    
      CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || 'EXCEPTION WHEN deadlock_detected THEN' ||CHR(10);
      CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || 'DATOS_BAD := 0;' ||CHR(10);
      CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || 'END;' ||CHR(10);
    
      CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || '   IF DATOS_BAD <> 0  THEN' ||CHR(10);
      ...
      CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO ||     CHR(10);
      CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || '       EXCEPTION WHEN OTHERS THEN' ||CHR(10);
      CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || '         PKG_UTILITARIOS.REPORTA_ERROR(''ERROR EN BAD: '', '' DESCRIPCION: '' || SQLERRM);' ||CHR(10);
      CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || '       END;' ||CHR(10);
      CLOB_PRC_DINAMICO     :=CLOB_PRC_DINAMICO || '   END LOOP;' ||CHR(10);
  ...
  SALIDA);*/
    ...
    ******************************************************************************/
    ...
    BEGIN 
      ...
      BEGIN
      null;
      PRC_CARGAR_ERRORES_SIBO(ID_RESUMEN, P_FECHA_ARCHIVO, GLOB_NOMBRE_TABLA_ERROR, GLOB_NOMBRE_TABLA_BAD);
      EXCEPTION WHEN OTHERS THEN
      PKG_UTILITARIOS.REPORTA_ERROR('ERROR CARGAR ERRORES',SQLERRM);
      END;
      
      BEGIN
      PRC_ACTUALIZA_RESUMEN_CARGUE(GLOB_NOMBRE_TABLA_STAGE, GLOB_NOMBRE_TABLA_ERROR, GLOB_NOMBRE_TABLA_BAD, ID_RESUMEN);
      EXCEPTION WHEN OTHERS THEN
      PKG_UTILITARIOS.REPORTA_ERROR('ERROR ACTUALIZAR RESUMEN',SQLERRM);
      END;
